...
setInterval(() => {
    ...
    db.run(`UPDATE tasks SET estado = 'Pendiente', started_at = NULL WHERE estado = 'En proceso' AND started_at < ?`, [limitTime], function(err) {
        if (!err && this.changes > 0) {
           console.log(`⏰ Cron Job: Reasignadas ${this.changes} tareas a Pendiente por tiempo excedido (> 1 min).`);
        }
    });
}, 5000); // El daemon escanea la base de datos cada 5 segundos.
...
// Ruta rápida para técnicos / botones (Inicia o finaliza la tarea)
