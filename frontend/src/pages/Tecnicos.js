    ...
    <div className="space-y-8 page-transition pb-20">
      ...
      <div className="max-w-4xl mx-auto md:mx-0 space-y-6">
        ...
        <div className="flex flex-col gap-2">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 ml-4 mb-1 italic">Disciplina Técnica:</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-1">
        {filteredTecnicos.map((tecnico) => (
          <div key={tecnico.id} className="premium-card p-5 group flex flex-col relative overflow-hidden bg-white hover:border-slate-300 transform hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex justify-between items-start mb-5 relative z-10">
              <div className="w-12 h-12 bg-slate-900 rounded-[1.2rem] flex items-center justify-center text-white shadow-xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6">
              ...
              <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all transform translate-y-1 group-hover:translate-y-0">
                <button onClick={(e) => { e.stopPropagation(); handleEdit(tecnico); }} className="p-2.5 bg-white text-slate-400 hover:text-secondary hover:shadow-lg rounded-xl transition-all border border-slate-100">
                  <Edit3 size={16} />
                </button>
                <button onClick={(e) => handleDelete(e, tecnico.id)} className="p-2.5 bg-white text-slate-400 hover:text-red-500 hover:shadow-lg rounded-xl transition-all border border-slate-100">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight group-hover:text-secondary transition-colors italic truncate">{tecnico.nombre}</h3>
                {tecnico.status === 'Inactivo' && <div className="w-2 h-2 rounded-full bg-red-400 animate-pulse"></div>}
              </div>
              
              <div className="flex items-center gap-2 mb-6">
                <div className="px-3 py-1 bg-secondary/5 rounded-full text-[9px] font-black text-secondary uppercase tracking-[0.2em] border border-secondary/10 italic">
              ...
              <div className="space-y-3 pt-5 border-t border-slate-50">
                <div className="flex items-center gap-3 text-slate-500 group-hover:text-slate-700 transition-colors">
                  <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-secondary/10 group-hover:text-secondary transition-all">
                    <Phone size={14} />
                  </div>
                  <span className="text-[11px] font-black font-mono tracking-widest">{tecnico.telefono}</span>
                </div>
                <div className="flex items-center gap-3 text-slate-300">
                  <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center text-slate-200">
                    <Award size={14} />
                  </div>
      ...
      {filteredTecnicos.length === 0 && (
        <div className="text-center py-24 bg-white rounded-[4rem] border-2 border-dashed border-slate-100 shadow-inner">
          <div className="mx-auto w-24 h-24 bg-slate-50 rounded-[2.5rem] flex items-center justify-center mb-8 text-slate-200 animate-bounce">
            <Wrench size={48} />
          </div>
          <h3 className="text-xl font-black text-slate-400 uppercase tracking-[0.3em]">Bóveda Vacía</h3>
          <p className="text-slate-300 text-xs mt-3 font-bold uppercase tracking-wider">No se encontraron especialistas en el sistema local</p>
        </div>
      )}
